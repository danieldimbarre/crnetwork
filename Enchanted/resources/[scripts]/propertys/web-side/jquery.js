window.addEventListener("message",function(event){
	switch(event["data"]["Action"]){
		...
		case "Weight":
			$("#weightTextLeft").html(`${(event["data"]["InvPeso"]).toFixed(3)}   /   ${(event["data"]["InvMax"]).toFixed(3)}`);
			$("#weightTextRight").html(`${(event["data"]["ChestPeso"]).toFixed(3)}   /   ${(event["data"]["ChestMax"]).toFixed(3)}`);
...
const Request = () => {
	$.post("http://propertys/Request",JSON.stringify({}),(data) => {
		$("#weightTextLeft").html(`${(data["InvPeso"]).toFixed(3)}   /   ${(data["InvMax"]).toFixed(3)}`);
		$("#weightTextRight").html(`${(data["ChestPeso"]).toFixed(3)}   /   ${(data["ChestMax"]).toFixed(3)}`);
