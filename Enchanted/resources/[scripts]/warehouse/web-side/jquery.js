window.addEventListener("message",function(event){
	switch(event.data.action){
		...
		case "updateWeight":
			...
			$("#weightTextRight").html(`${(event["data"]["warehousePeso"]).toFixed(3)}   /   ${(event["data"]["warehouseMaxpeso"]).toFixed(3)}`);
...
const Request = () => {
	$.post("http://warehouse/Request",JSON.stringify({}),(data) => {
		...
		$("#weightTextRight").html(`${(data["warehousePeso"]).toFixed(3)}   /   ${(data["warehouseMaxpeso"]).toFixed(3)}`);
