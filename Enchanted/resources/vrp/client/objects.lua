...
AddEventHandler("objects:Table",function(Table)
	...
	for Number = 1,120 do
		...
		local Blip = AddBlipForRadius(Table[Number]["Coords"]["x"],Table[Number]["Coords"]["y"],Table[Number]["Coords"]["z"],10.0)
...
-- TARGETLABEL
-----------------------------------------------------------------------------------------------------------------------------------------
function TargetLabel(Coords,Number,Mode,Weight)
	if Mode == "1" then
		exports["target"]:AddCircleZone("Objects:"..Number,vec3(Coords["x"],Coords["y"],Coords["z"] + Weight),0.75,{
			name = "Objects:"..Number,
			heading = 0.0
		},{
			shop = Number,
		...
		for Number,v in pairs(Objects) do
			if #(Coords - v["Coords"]) <= v["Distance"] then
				if not Init[Number] and LoadModel(v["Object"]) then
					if v["Mode"] then
						TargetLabel(v["Coords"],Number,v["Mode"],v["Weight"])
					end

					Init[Number] = CreateObjectNoOffset(v["Object"],v["Coords"],false,false,false)
					SetEntityHeading(Init[Number],v["Heading"])
...
function tvRP.ObjectControlling(Model)
	...
	if LoadModel(Model) then
		...
		while Progress do
			...
			DwText("~g~F~w~  CANCELAR",0.62)
			DwText("~g~E~w~  COLOCAR OBJETO",0.65)
			DwText("~y~PAGE UP~w~  PARA CIMA",0.68)
			DwText("~y~PAGE DOWN~w~  PARA BAIXO",0.71)
			DwText("~y~SCROLL UP~w~  GIRA ESQUERDA",0.74)
			DwText("~y~SCROLL DOWN~w~  GIRA DIREITA",0.77)
			DwText("~y~ARROW UP~w~  PARA LONGE",0.80)
			DwText("~y~ARROW DOWN~w~  PARA PERTO",0.83)
			DwText("~y~ARROW LEFT~w~  PARA ESQUERDA",0.86)
			DwText("~y~ARROW RIGHT~w~  PARA DIREITA",0.89)
