	...
	if not Information and not IsPauseMenuActive() then
		...
		while Information do
			...
			for Entitys,v in pairs(GetPlayers()) do
				...
				if Entitys ~= PlayerPedId() and Player(v)["state"]["Passport"] and not IgnoreF7[Player(v)["state"]["Passport"]] and HasEntityClearLosToEntity(Ped,Entitys,17) and #(Coords - OtherCoords) <= 5 then
