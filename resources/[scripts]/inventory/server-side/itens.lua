...
Use = {
	...
	["shielda"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		if not vRPC.inVehicle(source) then
			local Vehicle,Network,Plate,vehName = vRPC.VehicleList(source,4)
			if Vehicle then
				if vCLIENT.CheckCar(source,Vehicle) then
					...
					if PassportPlate then
						...
						if parseInt(#Datatable) > 0 then
							...
							if not Datatable["mods"]["16"] then
								Datatable["mods"]["16"] = -1
							end

							if Datatable["mods"]["16"] == -1 then
								if Datatable["mods"]["16"] >= vCLIENT.CheckMods(source,Vehicle,16) then
									TriggerClientEvent("Notify",source,"amarelo","Limite da <b>Blindagem</b> atingida.",5000)
								else
									vRPC.AnimActive(source)
									Active[Passport] = os.time() + 1000
									Player(source)["state"]["Buttons"] = true
									...
									if vTASKBAR.UpgradeVehicle(source) then
										...
										repeat
											if os.time() >= parseInt(Active[Passport]) then
												...
												if vRP.TakeItem(Passport,Full,1,false,Slot) then
													Datatable["mods"]["16"] = Datatable["mods"]["16"] + 1
													vCLIENT.ActiveMods(source,Network,Plate,16,Datatable["mods"]["16"])
													vRP.Query("entitydata/SetData",{ dkey = "Mods:"..PassportPlate["Passport"]..":"..vehName, dvalue = json.encode(Datatable) })
												end
											end
								...
								TriggerClientEvent("Notify",source,"amarelo","Modelo da <b>Blindagem</b> incorreta.",5000)
							end
						else
							TriggerClientEvent("Notify",source,"amarelo","Dirija-se até uma mecânica e efetue uma revisão.",5000)
						end
					...
					TriggerClientEvent("Notify",source,"amarelo","O veículo <b>"..VehicleName(vehName).."</b> não possui blindagem.",5000)
				end
			end
		end
	end,

	["shieldb"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		if not vRPC.inVehicle(source) then
			local Vehicle,Network,Plate,vehName = vRPC.VehicleList(source,4)
			if Vehicle then
				if vCLIENT.CheckCar(source,Vehicle) then
	...
	["shieldc"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		if not vRPC.inVehicle(source) then
			local Vehicle,Network,Plate,vehName = vRPC.VehicleList(source,4)
			if Vehicle then
				if vCLIENT.CheckCar(source,Vehicle) then
	...
	["shieldd"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		if not vRPC.inVehicle(source) then
			local Vehicle,Network,Plate,vehName = vRPC.VehicleList(source,4)
			if Vehicle then
				if vCLIENT.CheckCar(source,Vehicle) then
	...
	["shielde"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		if not vRPC.inVehicle(source) then
			local Vehicle,Network,Plate,vehName = vRPC.VehicleList(source,4)
			if Vehicle then
				if vCLIENT.CheckCar(source,Vehicle) then
	...
	-- 	if vRP.TakeItem(Passport,Full,1,true,Slot) then
	-- 		local Status = vRP.UserPremium(Passport)
