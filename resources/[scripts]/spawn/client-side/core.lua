...
-----------------------------------------------------------------------------------------------------------------------------------------
local Locate = {
	{ ["x"] = -2205.92, ["y"] = -370.48, ["z"] = 13.29, ["name"] = "Great Ocean Highway", ["hash"] = 1 },
	...
	if parseInt(#Characters) > 0 then
		for k,v in pairs(Characters) do
			if LoadModel(v["skin"]) then
				Peds[k] = CreatePed(4,v["skin"],pedCoords[k][1],pedCoords[k][2],27.19,209.77,false,false)
				...
				if LoadAnim("rcmbarry") then
					TaskPlayAnim(Peds[k],"rcmbarry","base",8.0,8.0,-1,49,0,0,0,0)
				end
